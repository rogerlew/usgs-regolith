...
subroutine derose(ulog,imax,chan_thresh,chan_depth,sc_rad,slope,slope_rad,&
  & dg2rad,contrib_area,plan_view_curv,soil_depth,trans_model,dif_ratio,&
  & depth_max,depth_min,C0,C2,zo,max_zones,power)
  implicit none
! LOCAL VARIABLES
...
! FORMAL ARGUMENTS
  integer, intent(in):: ulog,imax,max_zones,zo(imax)
	real, intent(in):: dif_ratio(max_zones),depth_max(max_zones),depth_min(max_zones)
  ...
  case('DRS2')
    ...
    do i=1,imax
      if(slope_rad(i) >= depth_min(zo(i))*dg2rad ) then ! slope angle was computed to radians in main program
        temp0=(C0(zo(i))-dif_ratio(zo(i))*slope(i))**power ! (C0-C1*SlopeAngle)^3 from DeRose (1996)
      else
        temp0=0.
      end if
!!      write(*,*) i, temp0
...
! Compare slope angle in channels with 0.2*critical slope angle and reduce thickness accordingly.
  ...
  case('DRS3')
    ...
    do i=1,imax
      if(slope_rad(i) >= depth_min(zo(i))*dg2rad ) then ! slope angle was computed to radians in main program
!!        temp0=(depth_max(zo(i))-C0(zo(i))*sign(1.,plan_view_curv(i)))*exp(-dif_ratio(zo(i))*slope(i)) ! Need to compute or import plan_view_curv array.
        temp0=(C0(zo(i))-C2(zo(i))*sign(1.,plan_view_curv(i)))*exp(-dif_ratio(zo(i))*slope(i)) ! Need to compute or import plan_view_curv array.
      else
        temp0=0.
      end if
      if(temp0>depth_min(zo(i))) then
...
!!        & exp(-dif_ratio(zo(i))*slope(i)), dif_ratio(zo(i)), slope(i)
! Compare slope angle in channels with 0.2*critical slope angle and reduce thickness accordingly.
      if(contrib_area(i)>chan_thresh) then 
         if(slope_rad(i)>0.2*sc_rad(zo(i))) then
            if(soil_depth(i)>chan_depth) soil_depth(i)=chan_depth ! Set to average alluvium depth.
  ...
  case default !('DRS1')
    ...
    do i=1,imax
      if(slope_rad(i) >= depth_min(zo(i))*dg2rad) then ! slope angle was computed to radians in main program
        temp0=C0(zo(i))*exp(-dif_ratio(zo(i))*slope(i)) ! C0*exp(-C1*SlopeAngle) from DeRose et al. 1991
      else
        temp0=0.
      end if
!!      write(*,*) i, temp0, zo(i), dif_ratio(zo(i)), slope(i), depth_min(zo(i))
      if(temp0 > depth_min(zo(i))) then
        soil_depth(i)=temp0
!!        write(*,*) '+', soil_depth(i)
      else
